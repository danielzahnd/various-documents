...
\begin{enumerate}
	\item The material must be an ideal conductor, meaning that its resistivity $\mathcal{R}$ is zero, i.e. $\mathcal{R} = 0$.
	\item The material must be an ideal diamagnet, meaning that a magnetic field $-\vect{B}$ forms within the material in response to an external magnetic field $\vect{B}$, thus exactly cancelling out the residual magnetic field.
